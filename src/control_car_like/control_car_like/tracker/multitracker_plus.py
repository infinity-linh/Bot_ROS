...
class JDETracker(object):
    def __init__(self, model_yolo, conf_thres, track_buffer, frame_rate=30):
    ...
    def update(self, im_blob, img_src, iou_thres, classes, agnostic_nms, max_det):
        self.frame_id += 1
        ...
        if len(pred) > 0:
            dets = non_max_suppression(pred, self.det_thresh, iou_thres, classes, agnostic_nms, max_det=max_det)[0]
            dets[:, :4] = rescale(im_blob.shape[2:], dets[:, :4], img_src.shape).round()
