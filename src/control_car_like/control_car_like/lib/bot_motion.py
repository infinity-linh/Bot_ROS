...
class bot_motionplanner():
    ...
    def get_pose(self,data):
        ...
        # 1) We get the bot_turn_angle in simulation Using same method as Gotogoal.py
        ...
        print("self.actual_speed = ",self.actual_speed)
    ...
    def display_control_mechanism_in_action(self,bot_loc,path,img_shortest_path,bot_localizer,frame_disp):
        ...
        if config.debug and config.debug_motionplanning:
            # cv2.namedWindow("maze (Shortest Path + Car Loc)",cv2.WINDOW_NORMAL)
            cv2.imshow("maze (Shortest Path + Car Loc)",img_shortest_path)
            # cv2.imshow("CONFIG",img_shortest_path)
    ...
    def angle_n_dist(pt_a,pt_b):
        ...
        angle = atan2(error_x,error_y)
        # Converting angle from radians to degrees
        angle_deg = degrees(angle)
        # return angle_deg, distance
        if angle_deg > 0:
            print("###############_Angle(+): ", angle_deg)
            return angle_deg - 180, distance
        else:
            print("###############_Angle(-): ", angle_deg)
            return angle_deg + 180, distance
        # if (angle_deg>0):
    ...
    def go_to_goal(self,bot_loc,path):
        ...
        speed = interp(distance_to_goal,[0,100],[0,2.5])

        angle_to_turn = -angle_to_goal + self.bot_angle

        angle = interp(angle_to_turn,[-20,20],[-60,60])
        # if abs(angle_to_goal) > 90:
        #     self.vel_angular_z = 0.0
        #     self.vel_linear_x = -10.0
        if distance_to_goal > 100:
            self.vel_linear_x = 0.0
        # elif abs(angle_to_turn) < 1:
            # self.vel_angular_z = 0.0
        else:
            self.vel_linear_x = speed
            self.vel_angular_z = angle
            self.angle_bot_pre = angle_to_goal
        self.bot_angle = self.angle_bot_pre

        
        if (config.debug and config.debug_motionplanning):
            # print("angle to goal = {} Angle_to_turn = {} angle[Sim] {}".format(angle_to_goal,angle_to_turn, angle))
            # print("distance_to_goal = ",distance_to_goal)
            print("angle to car", self.bot_angle)
            print("Phi", angle_to_turn)
            print("angle to goal",angle_to_goal)
            print("distance", distance_to_goal)
        # If large turn to goal. then make car turn according to req by goal and speed to 1
        if not self.goal_not_reached_flag and distance_to_goal <=50:
            self.vel_linear_x = 0.0
            self.vel_angular_z = 0.0

        # If car is within reasonable distance of mini-goal
        if (distance_to_goal<=30): 
            # self.bot_angle = angle_to_goal
    ...
    def nav_path(self,bot_loc,bot_loc_sv,path):
        ...
        if (self.count >20):
            ...
            if not self.angle_relation_computed:
                ...
                print("(X, Y) = ", (self.goal_pose_x,self.goal_pose_y))
                print("")
                # print(self.bot_angle_s)
                # Traversing through found path to reach goal
                self.go_to_goal(bot_loc,path)

        else:

            # Only proceed if bot lies within the road network
            if all(bot_loc_sv>0):
                # If bot initial location not already taken
                if not self.pt_i_taken:
                    print("")
                    print("4# Navigating to Destination........")
