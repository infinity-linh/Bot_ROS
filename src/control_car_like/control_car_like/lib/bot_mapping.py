...
class bot_mapper():
    ...
    def graphify(self,extracted_maze,bot_loc=[],destination=[],car_rect=[]):
        ...
        if not self.graphified:
            ...
            if bot_loc!=[]:
                road_cnts = cv2.findContours(thinned_cropped, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)[0]
            ...
            [x,y,w,h] = car_rect
            cnts_dcsn_pts = cv2.findContours(self.maze_dcsn_pts, cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE)[0]
            cntr_lst = []
